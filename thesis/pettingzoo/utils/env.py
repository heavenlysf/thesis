"""
Base env_runner definitions

See docs/api.md for api documentation
See docs/dev_docs.md for additional documentation and an example env_runner.
"""
...
class AECEnv:
    ...
    def reset(self):
        """
        resets the env_runner and returns a dictionary of observations (keyed by the agent name)
        """
        raise NotImplementedError

    def seed(self, seed=None):
        """
        Reseeds the env_runner (making the resulting env_runner deterministic).
    ...
    def render(self, mode="human"):
        """
        Displays a rendered frame from the env_runner, if supported.
    ...
    def state(self):
        """
        State returns a global view of the env_runner appropriate for
        centralized training decentralized execution methods like QMIX
        """
        raise NotImplementedError(
            "state() method has not been implemented in the env_runner {}.".format(
...
class ParallelEnv:
    """
    The Parallel env_runner steps every live agent at once. If you are unsure if you
    ...
    def seed(self, seed=None):
        """
        Reseeds the env_runner (making it deterministic).
        `reset()` must be called after `seed()`, and before `step()`.
        """
        pass
