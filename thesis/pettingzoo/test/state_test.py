...
def test_state_space(env):
    ...
    ), "State space for each env_runner must extend gym.spaces.Space"
    if not (
        isinstance(env.state_space, gym.spaces.Box)
        or isinstance(env.state_space, gym.spaces.Discrete)
    ):
        warnings.warn(
            "State space for each env_runner probably should be gym.spaces.box or gym.spaces.discrete"
        )
...
def test_parallel_env(parallel_env):
    ...
    ), "State space for each parallel env_runner must extend gym.spaces.Space"
