...
def test_observation_action_spaces(env, agent_0):
    ...
    for agent in env.agents:
        ...
        if (not isinstance(agent, str)) and agent != "envs":
            warnings.warn(
...
def play_test(env, observation_0, num_cycles):
    """
    plays through env_runner and does dynamic checks to make
    sure the state returned by the env_runner is
    ...
    * tests that the observation is in bounds.
    ...
    for agent in env.agent_iter(env.num_agents * num_cycles):
        ...
        ), "an env_runner agent's info must be a dictionary"
        ...
        ), "agent cannot be done twice in an env_runner"
        assert env.num_agents == len(
            env.agents
        ), "envs.num_agents is not equal to len(envs.agents)"
        ...
        ), "env_runner must delete agents as the game continues"
        if not env.agents:
...
def api_test(env, num_cycles=10, verbose_progress=False):
    ...
    assert env.num_agents != 0, "An env_runner should have a nonzero number of agents"
    assert env.num_agents > 0, "An env_runner should have a positive number of agents"
    ...
    # Test that if envs has overridden render(), they must have overridden close() as well
