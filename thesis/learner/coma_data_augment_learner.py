...
class ComaDataAugmentLearner(ILearner):
    ...
    def train(self):
        ...
        # update critic
        actions = actions.repeat_interleave(self.n_duplicate, dim=1)
        log_critic_loss = []
        residual_variance_log_1 = []
        residual_variance_log_2 = []
        q_values = []
        for t in reversed(range(self.max_step)):
            duplicate_obs = np.array(observations[t])
            duplicate_obs = np.array([random.sample(list(permutations(x)), k=self.n_duplicate) for x in duplicate_obs])
            duplicate_obs = torch.FloatTensor(duplicate_obs)
            q_t = self.critic(duplicate_obs)
            ...
            return_t = return_t.repeat_interleave(self.n_duplicate, dim=0)
        ...
        # baseline
        observations = observations.repeat_interleave(self.n_duplicate, dim=1)
        probs = probs.repeat_interleave(self.n_duplicate, dim=1)
